...
ICONS = {
 ...
 "sim": '<path d="M6 3h8l4 4v14a1 1 0 01-1 1H6a1 1 0 01-1-1V4a1 1 0 011-1z"/><rect x="8.5" y="11" width="7" height="6" rx="1"/><path d="M8.5 14h7M12 11v6"/>',
...
def build(slug, device, c):
    ...
    SPOKE_LINKS = {
        ...
        # AirPods: 18.08 кластер давал 216 показов и НОЛЬ кликов при позиции 21,1 —
        # сервисных страниц не было вовсе. Все десять строк прайса ведут на три спойка.
        "remont-airpods":     {"Чистка наушников и кейса от серы и грязи": ("chistka/", "Чистка AirPods — цены"),
                               "Замена сеток динамиков": ("chistka/", "Замена сеток динамиков AirPods"),
                               "Замена амбушюр (AirPods Pro)": ("chistka/", "Замена амбушюр AirPods Pro"),
                               "Чистка после попадания влаги, сушка": ("chistka/", "AirPods после влаги — чистка"),
                               "Замена аккумулятора наушника": ("zamena-akkumulyatora/", "Замена аккумулятора AirPods"),
                               "Замена аккумулятора кейса": ("zamena-akkumulyatora/", "Замена аккумулятора кейса AirPods"),
                               "Замена зарядного кейса (футляра)": ("zamena-akkumulyatora/", "Замена кейса AirPods"),
                               "Восстановление зарядки (чистка/ремонт контактов)": ("zamena-akkumulyatora/", "AirPods не заряжаются — что делать"),
                               "Замена одного наушника (левый/правый)": ("ne-rabotaet-naushnik/", "Замена одного наушника AirPods"),
                               "Ремонт платы наушника (звук/подключение)": ("ne-rabotaet-naushnik/", "Ремонт платы наушника AirPods")},
        # iPad: 87 показов и ноль кликов, но позиция уже 8,3 — ранжирование есть,
        # не было цели. Девять строк прайса разведены по трём спойкам.
        "remont-ipad":        {"Замена тачскрина (сенсорного стекла)": ("zamena-stekla/", "Замена стекла iPad — цены"),
                               "Замена дисплея": ("zamena-stekla/", "Замена дисплея iPad"),
                               "Замена корпуса": ("zamena-stekla/", "Замена корпуса iPad"),
                               "Замена аккумулятора": ("zamena-akkumulyatora/", "Замена аккумулятора iPad — цены"),
                               "Замена разъёма зарядки": ("ne-vklyuchaetsya/", "iPad не заряжается — замена разъёма"),
                               "Чистка разъёма зарядки": ("ne-vklyuchaetsya/", "Чистка разъёма зарядки iPad"),
    ...
    CARD_LINKS = {"remont-apple-watch": {
        ...
        "Чистка от грязи и серы":       ("chistka/", "Чистка AirPods — цены и сроки"),
        "Тихий или пропадающий звук":   ("chistka/", "Почему AirPods стали тише"),
        "Замена амбушюр и сеток":       ("chistka/", "Амбушюры и сетки — что почём"),
        "После попадания влаги":        ("chistka/", "AirPods намокли — что делать"),
        "Не заряжаются AirPods":        ("zamena-akkumulyatora/", "AirPods не держат заряд — причины"),
        "Замена аккумулятора наушника": ("zamena-akkumulyatora/", "Замена аккумулятора AirPods — цены"),
        "Замена кейса (футляра)":       ("zamena-akkumulyatora/", "Замена кейса AirPods"),
        "Проблемы с подключением":      ("ne-rabotaet-naushnik/", "Не работает один наушник — что делать"),
    }, "remont-ipad": {
        "Замена тачскрина (сенсорного стекла)": ("zamena-stekla/", "Замена стекла iPad — цены и сроки"),
        "Замена дисплея":            ("zamena-stekla/", "Стекло или дисплей — в чём разница"),
        "Замена корпуса":            ("zamena-stekla/", "Замена корпуса iPad"),
        "Замена аккумулятора":       ("zamena-akkumulyatora/", "Замена аккумулятора iPad — цены"),
        "Замена разъёма зарядки":    ("ne-vklyuchaetsya/", "iPad не заряжается — причины"),
        "Ремонт после воды":         ("ne-vklyuchaetsya/", "iPad после воды — что делать"),
        "Замена кнопок Home / Power":("ne-vklyuchaetsya/", "Не нажимается кнопка iPad"),
        "Ремонт материнской платы":  ("ne-vklyuchaetsya/", "Ремонт платы iPad"),
    }}
    ...
    page += '        <div class="trustbar"><span class="tb-star">★ 4.8</span> <b>Google</b><span class="sep">·</span>158 отзывов<span class="sep">·</span><b>32 000</b> ремонтов<span class="sep">·</span>8 лет</div>\n'
    ...
    if _subs:
        _CTA = '<span class="mgo">Цены и виды ремонта <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M5 12h13M12 5l7 7-7 7"/></svg></span>'
        _mlinks = "\n        ".join('<a class="mcard mcard-cat" href="%s/"><span class="nm">%s</span>%s</a>'%(esc(x["slug"]), esc(x["name"]), _CTA) for x in _subs)
    ...
    <div class="wrap">
      ...
      <div class="why-grid">
        ...
        <div class="why reveal"><div class="ic"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="8" r="4"/><path d="M4 21c0-4 4-6 8-6s8 2 8 6"/></svg></div><h3>Опытные мастера</h3><p>8 лет на рынке Одессы, более 32 000 выполненных ремонтов.</p></div>
      </div>
    </div>
  </section>\n\n'''%esc(device)
    # seo text
