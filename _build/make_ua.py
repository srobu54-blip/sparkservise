...
# FAQPage годами не совпадал с видимым <summary> (нарушение требования Google),
# а всего таких расхождений накопилось 10. Возникают они незаметно: ветки
# наполняются независимо (head переводили одним проходом, body другим), а
# i18n_bootstrap.put() ловит конфликт только ВНУТРИ одной ветки.
# Ниже — осознанные исключения; всё остальное печатается как предупреждение.
BRANCH_DIFF_OK = {
    # суффикс сайта уместен в <title> (text), но не в BlogPosting.headline (jsonld)
    'iPhone упал в воду: что делать в первые 10 минут',
    # коллизия ролей: одна RU-строка обслуживает и Service.name, и мелкий eyebrow
    'Диагностика в Одессе',
    # attr = <meta description>, намеренно подрезан под лимит выдачи (~150 симв.),
    # text/jsonld несут полную формулировку — это правильно, не выравнивать
    'Разбираем, почему iPhone быстро садится: износ батареи, фоновые процессы, мороз, сеть. Как проверить состояние АКБ и при каком проценте пора менять.',
    'iPhone не заряжается или заряжается медленно? Разбираем причины: кабель, разъём, iOS, контроллер питания. Самопомощь и ремонт в независимом сервисе в Одессе.',
    # легаси: RU-строки на сайте уже нет; в attr сидит оптимизированный под Ahrefs
    # вариант «Сервісний центр Apple в Одесі» (запрос «сервісний центр apple», 150/мес)
    'Ремонт Apple в Одессе - iPhone, Mac, iPad | SPARK',
    # <meta name="keywords"> — Google их не учитывает; в attr добавлен гео-хвост
    'iphone не заряжается, почему не заряжается айфон, айфон не заряжается до конца, iphone не заряжается от зарядки, iphone заряжается медленно, чистка разъёма зарядки iphone, ремонт iphone одесса, контроллер питания iphone',
    'iphone упал в воду, что делать если iphone попал в воду, утопил айфон, iphone намок, сушка iphone, чистка платы iphone, ремонт iphone после воды, окисление платы, спасти айфон от воды',
    'почему iphone быстро разряжается, состояние аккумулятора iphone, замена батареи iphone, износ акб, iphone садится на холоде, проверить аккумулятор айфона, замена аккумулятора одесса',
}

def check_branch_consistency():
    seen = {}
    for br in ('text', 'attr', 'jsonld'):
        for k, v in CAT[br].items():
            seen.setdefault(k, {})[br] = v
    bad = [(k, d) for k, d in seen.items()
           if len(set(d.values())) > 1 and k not in BRANCH_DIFF_OK]
    if not bad:
        print("Согласованность веток каталога: OK (исключений в белом списке: %d)"
              % len(BRANCH_DIFF_OK))
        return
    print("⚠ ОДИН RU-КЛЮЧ → РАЗНЫЕ UA-ПЕРЕВОДЫ в ветках: %d" % len(bad))
    for k, d in bad[:15]:
        print("  RU: %r" % k[:70])
        for br, v in d.items():
            print("     %-7s %r" % (br, v[:70]))
