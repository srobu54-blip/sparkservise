...
# 4.12) Сервисные страницы AirPods (3 шт. из одного генератора). По GSC за
#      11-17.08 кластер AirPods давал 216 показов и НОЛЬ кликов при позиции
#      21,1: десять услуг в прайсе и ни одной страницы под них. Три страницы,
...
# 4.14) Сервисные страницы Apple Watch (2 шт., та же вёрстка). Кластер — 142
#      показа, но его единственная страница /zamena-stekla/ даёт CTR 9,4%, лучший
#      на сайте: ниша не мёртвая, а недообслуженная. Две страницы разбирают
#      девять оставшихся строк прайса; на запрос «замена стекла» по-прежнему
#      отвечает старая страница, новая страница экрана уводит дешёвые случаи ей.
step "assemble_watch — Apple Watch";           $PY _build/assemble_watch.py
